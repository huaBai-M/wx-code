 ...
 */
  ...
  onSyncDone() {
    ...
    // wx.navigateTo({
    //   url: '/partials/personcard/personcard'
    // })
  }
  /**
 ...
 */
  ...
  onMsg(msg) {
    ...
    try {
      ...
      console.log(app.globalData.nim.protocol.myInfo)
    } catch (error) {
          ...
          wx.showModal({
            ...
            success: (res) => {
              if (res.confirm) { //点击确定
                ...
                if (currentPage.route.includes('videoCallMeeting')) { // 多人
                  try {
                    // 兼容登录网关502错误离开房间
  ...
  */
  ...
  onOfflineMsgs(msg) {
    console.log("离线消息离线消息离线消息离线消息离线消息离线消息离线消息离线消息离线消息离线消息离线消息离线消息")
    console.log(orderCounter++, ' onOfflineMsgs')
    app.globalData.messageNumber=msg.msgs.length
