...
let pageConfig = {
  ...
  onLoad: function (options) {
    ...
    this.setData({
      account: userData.token,
      password: userData.accid,
      toAccid: 1
    });
    new IMController({
      token: this.data.account,
      account: this.data.password
    })
    /*登录im结束*/
    this.setData({
      iconNoMessage,
  ...
  Toupdate() {
    let cardData = globalData.cardData;
    let userData = wx.getStorageSync('userData');
    console.log(cardData)
    if (globalData.iftoAccid) {
      if (cardData == null) {
        app.getCardfun(globalData.wxCardEmployeeId, globalData.companyId, userData.openId).then((res) => {
          console.log(res)
          globalData.cardData = res.data.data;
          this.addFriend()
        })
      } else {
        let account = cardData.imAccid;
        //添加好友
        this.addFriend()

        // this.oneChating("888", "p2p-888")
      }
    }
  },
  addFriend() {
    let account = globalData.cardData.imAccid;
    let userData = wx.getStorageSync('userData');
    wx.request({
      url: app.globalData.webRequsetUrl + '/weixin/addFriend',
      method: 'post',
      header: { 'content-type': 'application/x-www-form-urlencoded' }, // 默认值
      data: {
        fromAccid: userData.accid,//访客AccID 
        toAccid: account //名片AccID
      },
      success: res => {
        console.log("添加好友成功")
        console.log("跳")
        //this.oneChating(account, "p2p-" + account)
        setTimeout(() => {
          this.oneChating(account, "p2p-" + account)
        }, 1000)
  ...
  addNotificationToChatList(msg) {
    ...
    if (!self.data.chatAccount['notification']) { // 没有系统通知
      ...
      console.log("**************")
      console.log(self.data.chatList)
    }
  },
  /**
   * 捕获从滑动删除传递来的事件
