...
let pageConfig = {
  data: {
    ...
    videoContext: null, // 操纵对象
    isVideoFullScreen: false, // 全屏控制标准
    videoSrc: '', // 源
  ...
  formSubmit(val) {
    ...
    wx.request({
      ...
      data: {
        visitorId: app.globalData.visitorId,//访客id
  ...
  onLoad: function (options) {
    ...
    if (chatTo === this.data.userInfo.account) {
      ...
      wx.setNavigationBarTitle({
        title: globalData.cardData.name,
      })
    }
    this.setData({
      chatTo,
  ...
  inputSend(e) {
    ...
    wx.request({
      ...
      data: {
        ...
        loginId: globalData.cardData.loginId,//账户id
        sfaCompanyId: globalData.sfaCompanyId,
        loginType: globalData.cardData.loginType,//账户类型
        wxCardEmployeeId: globalData.cardData.id,//名片id
  ...
  videoCall() {
    ...
    //   }
    // } else { // p2p
    //   console.log(`正在发起对${this.data.chatTo}的通话`)
    //   wx.navigateTo({
